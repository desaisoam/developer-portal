...
/**
 * Eight local-midnight ISO boundaries producing seven day buckets
 * `[bounds[i], bounds[i + 1])` for the trailing week ending today:
 *   - bounds[6] = today's local midnight
 *   - bounds[0] = six days before today's midnight (also the "this week" lower bound)
 *   - bounds[7] = tomorrow's local midnight (today's bucket is [bounds[6], bounds[7]))
 *
 * Boundaries are cut at the viewer's local midnight and emitted as UTC ISO
 * strings so they can be passed straight into Hasura `timestamptz` range
 * variables and also drive the sparkline buckets.
 */
...
/**
 * Eight ISO boundaries producing seven equal-width buckets across an entity's
 * lifetime. The final boundary is tomorrow's local midnight so the current
 * bucket keeps accepting new rows while the page is open.
 */
...
/** Human-readable tooltip labels for the seven chart buckets. */
export const trendBucketLabels = (
  bounds: string[],
  period: TrendPeriod,
): string[] => {
  if (period === "weekly") {
    // "Thu · Jul 9" — enough for the hover tooltip to be unambiguous without
    // implying a finer granularity than the day bucket actually has.
