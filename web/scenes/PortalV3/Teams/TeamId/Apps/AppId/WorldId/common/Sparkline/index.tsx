...
type SparklineProps = {
  // One value per time bucket (oldest first). Rendered as a normalised polyline.
  points: number[];
  // Optional per-point labels; when present the chart becomes interactive —
  // hovering shows a dashed guide line and a tooltip with the bucket's count
  // and label (e.g. "185 verifications / Thu · Jul 9").
  labels?: string[];
  className?: string;
  ariaLabel?: string;
};
...
export const Sparkline = ({
  ...
  // Keep the tooltip roughly on-card at the extremes instead of centering it
    ...
    >
      ...
      {interactive && hover !== null ? (
        <>
          ...
          />
          <div
            className="pointer-events-none absolute bottom-full z-20 mb-2 flex flex-col gap-0.5 whitespace-nowrap rounded-8 border border-portal-border bg-white px-2.5 py-1.5 shadow-portal-card"
            style={{
              left: `${hoverLeftPct}%`,
